...
- [x] **8.2** Add production AWS certificate chain verification
  - [x] COSE Sign1 signature verification for attestation documents
  - [x] X.509 certificate chain parsing and validation
  - [x] P-384 ECDSA signature verification using `p384` crate
  - [x] Sig_structure building per RFC 8152
- [x] **8.3** Create real PCR measurement extraction and validation
  - [x] `extract_pcrs_from_payload()` parses PCRs from COSE payload
  - [x] `verify_pcr_measurements_from_payload()` cross-validates claimed vs signed
  - [x] Integrated into attestation verification pipeline
